...
class GwyObject(OrderedDict):
    ...
    @classmethod
    def fromnrrd(cls, file):
        """
        Trys to create a GwyObject from an NRRD or NHRD file
        in Testing

        Currently onnly accepting 3D NRRD Data. 
        NRRD File must have sizes, units and sampleunits Tag!

        Parameters
        ----------
        file: nrrd or nhrd  fileobject or filename
        """
        ...
        obj["/brick/0"]= GwyBrick(data.astype(float), xres=header["sizes"][0], yres=header["sizes"][1] ,zres=header["sizes"][2], 
                                    ...
                                    si_unit_z=GwySIUnit(unitstr=header["units"][2]),
                                    si_unit_w=GwySIUnit(unitstr=header["sampleunits"]))
        try:
            obj["/brick/0/title"]=header["content"]
        except:
